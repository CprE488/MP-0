...
void print_board()
{
	...
	for(y=0; y<BOARD_HEIGHT_CELLS; y++)
	{
		for(x=0; x<BOARD_WIDTH_CELLS; x++)
		{
			if(board[y][x].piece)
			{
				if(board[y][x].piece->fg_color == SetPixel(0x0, 0x0, 0x0))
					printf("Q");
				else
					printf("q");
			}
			...
			{
				if(board[y][x].bg_color == SetPixel(0xA, 0x2, 0x3))
					printf(" ");
				else
					printf("#");
			}
...
void set_queen_colors(Pixel safe, Pixel danger)
{
	int i;
	Cell *cell;
	for(i=0;i<8;i++)
	{
		queens[i].fg_color = safe;
		queens[i].danger_count = 0;
	}
	for(i=0;i<8;i++)
	{
		for(cell=queens[i].location;cell;cell=cell->neighbors.north)
		{
			if(cell->piece)
			{
				cell->piece->fg_color = danger;
				cell->piece->danger_count++;
			}
		}
		for(cell=queens[i].location;cell;cell=cell->neighbors.north_east)
		{
			if(cell->piece)
			{
				cell->piece->fg_color = danger;
		...
		for(cell=queens[i].location;cell;cell=cell->neighbors.east)
		{
			if(cell->piece)
			{
				cell->piece->fg_color = danger;
		...
		for(cell=queens[i].location;cell;cell=cell->neighbors.south_east)
		{
			if(cell->piece)
			{
				cell->piece->fg_color = danger;
		...
		for(cell=queens[i].location;cell;cell=cell->neighbors.south)
		{
			if(cell->piece)
			{
				cell->piece->fg_color = danger;
		...
		for(cell=queens[i].location;cell;cell=cell->neighbors.south_west)
		{
			if(cell->piece)
			{
				cell->piece->fg_color = danger;
		...
		for(cell=queens[i].location;cell;cell=cell->neighbors.west)
		{
			if(cell->piece)
			{
				cell->piece->fg_color = danger;
		...
		for(cell=queens[i].location;cell;cell=cell->neighbors.north_west)
		{
			if(cell->piece)
			{
				cell->piece->fg_color = danger;
...
int get_danger_count(Cell *cell_to_test)
{
	int danger_count = 0;
	Cell *cell;
	for(cell=cell_to_test;cell;cell=cell->neighbors.north)
		if(cell->piece)
			danger_count++;
	for(cell=cell_to_test;cell;cell=cell->neighbors.north_east)
		if(cell->piece)
			danger_count++;
	for(cell=cell_to_test;cell;cell=cell->neighbors.east)
		if(cell->piece)
			danger_count++;
	for(cell=cell_to_test;cell;cell=cell->neighbors.south_east)
		if(cell->piece)
			danger_count++;
	for(cell=cell_to_test;cell;cell=cell->neighbors.south)
		if(cell->piece)
			danger_count++;
	for(cell=cell_to_test;cell;cell=cell->neighbors.south_west)
		if(cell->piece)
			danger_count++;
	for(cell=cell_to_test;cell;cell=cell->neighbors.west)
		if(cell->piece)
			danger_count++;
	for(cell=cell_to_test;cell;cell=cell->neighbors.north_west)
		if(cell->piece)
			danger_count++;
	return danger_count;
}
...
u8 run_step()
{
	int i, best = queens[0].danger_count , best_queen_index=0, best_option_index=0;
	int dir = rand(0,4);
	...
	struct OPTION_STRUCT options[7];

	Cell *cell;
	for(i=1;i<8;i++)
	{
		best = queens[i].danger_count > best ? best_queen_index=i, queens[i].danger_count : best;
	}
	...
	{
	case 0:
		for(cell=queens[best_queen_index].location->neighbors.north; cell; cell=cell->neighbors.north)
		{
			options[i].cell = cell;
			options[i].danger_count = get_danger_count(cell);
			i++;
		}
		for(cell=queens[best_queen_index].location->neighbors.south; cell; cell=cell->neighbors.south)
		{
			options[i].cell = cell;
			options[i].danger_count = get_danger_count(cell);
			i++;
		...
		break;
	case 1:
		for(cell=queens[best_queen_index].location->neighbors.north_east; cell; cell=cell->neighbors.north_east)
		{
			options[i].cell = cell;
			options[i].danger_count = get_danger_count(cell);
			i++;
		}
		for(cell=queens[best_queen_index].location->neighbors.south_west; cell; cell=cell->neighbors.south_west)
		{
			options[i].cell = cell;
			options[i].danger_count = get_danger_count(cell);
			i++;
		...
		break;
	case 2:
		for(cell=queens[best_queen_index].location->neighbors.east; cell; cell=cell->neighbors.east)
		{
			options[i].cell = cell;
			options[i].danger_count = get_danger_count(cell);
			i++;
		}
		for(cell=queens[best_queen_index].location->neighbors.west; cell; cell=cell->neighbors.west)
		{
			options[i].cell = cell;
			options[i].danger_count = get_danger_count(cell);
			i++;
		...
		break;
	case 3:
		for(cell=queens[best_queen_index].location->neighbors.north_west; cell; cell=cell->neighbors.north_west)
		{
			options[i].cell = cell;
			options[i].danger_count = get_danger_count(cell);
			i++;
		}
		for(cell=queens[best_queen_index].location->neighbors.south_east; cell; cell=cell->neighbors.south_east)
		{
			options[i].cell = cell;
			options[i].danger_count = get_danger_count(cell);
			i++;
	...
	best = options[0].danger_count;
	best_option_index = 0;
	for(i=1;i<7;i++)
	{
		best = options[i].danger_count < best ? best_option_index = i, options[i].danger_count : best;
	}
	queens[best_queen_index].location->piece = 0;
	queens[best_queen_index].location = options[best_option_index].cell;
	options[best_option_index].cell->piece = &queens[best_queen_index];
